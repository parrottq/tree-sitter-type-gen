    ...
    Self::Container<NodeOrAny<'a, Self::Child>>: DeserializeNode<'a>,
{
    ...
    fn children_field(&self, field_id: u16) -> Result<Vec<Self::Child>, DeserializeError> {
...
pub fn default_deserialize_at_root<'a, T>(
    tree: &mut TreeCursor<'a>,
    mode: DeserializeMode,
) -> Result<T, DeserializeError>
where
...
pub fn default_deserialize_at_current<'a, T>(
    iter: &mut Peekable<impl Iterator<Item = Node<'a>>>,
) -> Result<T, DeserializeError>
where
    ...
    <T as GenericNode<'a>>::Container<<T as GenericNode<'a>>::Child>: DeserializeNode<'a>,
...
pub fn variants_deserialize_at_current<'a, const N: usize, T, I>(
    iter: &mut Peekable<I>,
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match &mut self.0 {
            ...
            IntU16Set::Range(lower, upper) => {
                ...
                debug_assert!(
                    lower < upper,
                    "Upper range values should always be larger than lower range"
                );
